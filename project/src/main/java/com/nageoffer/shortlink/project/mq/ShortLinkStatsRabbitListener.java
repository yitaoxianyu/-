...
public class ShortLinkStatsRabbitListener {
    ...
    @RabbitListener(queues = ShortLinkConstants.MQ_STATS_QUEUE)
    public void onMessage(Map<String,String> map, @Header(AmqpHeaders.MESSAGE_ID) String messageId,
                          Channel channel,@Header(AmqpHeaders.DELIVERY_TAG) long deliveryTag){
        asyncExecutor.submit(() -> {
            if (idempotentUtil.isProcessed(messageId)) {
                //这里直接 ack 不重新投递
                try {
